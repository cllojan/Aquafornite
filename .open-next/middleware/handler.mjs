...
var require_middleware = __commonJS({
  ".next/server/src/middleware.js"() {
    ...
    (self.webpackChunk_N_E = self.webpackChunk_N_E || []).push([[550], { 165: (a, b, c) => {
      ...
      c.r(f), c.d(f, { config: () => bF, middleware: () => bE });
      let h = null;
      async function i() {
...
Learn More: https://nextjs.org/docs/messages/node-module-in-edge-runtime`;
  ...
  `);
      ...
      class bg extends Error {
        constructor(a10, b2) {
          super(a10), this.name = "BetterAuthError", this.message = a10, this.cause = b2, this.stack = "";
        }
      }
      let bh = /* @__PURE__ */ Object.create(null), bi = (a10) => globalThis.process?.env || globalThis.Deno?.env.toObject() || globalThis.__env__ || (a10 ? bh : globalThis), bj = new Proxy(bh, { get: (a10, b2) => bi()[b2] ?? bh[b2], has: (a10, b2) => b2 in bi() || b2 in bh, set: (a10, b2, c2) => (bi(true)[b2] = c2, true), deleteProperty(a10, b2) {
        if (!b2) return false;
        let c2 = bi(true);
        return delete c2[b2], true;
      }, ownKeys: () => Object.keys(bi(true)) });
      function bk(a10, b2) {
        return "undefined" != typeof process && process.env ? process.env[a10] ?? b2 : "undefined" != typeof Deno ? Deno.env.get(a10) ?? b2 : "undefined" != typeof Bun ? Bun.env[a10] ?? b2 : b2;
      }
      "undefined" != typeof process && process.env;
      let bl = { eterm: 4, cons25: 4, console: 4, cygwin: 4, dtterm: 4, gnome: 4, hurd: 4, jfbterm: 4, konsole: 4, kterm: 4, mlterm: 4, mosh: 24, putty: 4, st: 4, "rxvt-unicode-24bit": 24, terminator: 24, "xterm-kitty": 24 }, bm = new Map(Object.entries({ APPVEYOR: 8, BUILDKITE: 8, CIRCLECI: 24, DRONE: 8, GITEA_ACTIONS: 24, GITHUB_ACTIONS: 24, GITLAB_CI: 8, TRAVIS: 8 })), bn = [/ansi/, /color/, /linux/, /direct/, /^con[0-9]*x[0-9]/, /^rxvt/, /^screen/, /^xterm/, /^vt100/, /^vt220/], bo = { reset: "\x1B[0m", bright: "\x1B[1m", dim: "\x1B[2m", fg: { red: "\x1B[31m", green: "\x1B[32m", yellow: "\x1B[33m", blue: "\x1B[34m", magenta: "\x1B[35m" } }, bp = ["info", "success", "warn", "error", "debug"], bq = { info: bo.fg.blue, success: bo.fg.green, warn: bo.fg.yellow, error: bo.fg.red, debug: bo.fg.magenta };
      function br(a10, b2 = "/api/auth") {
        if (function(a11) {
          try {
            let b3 = new URL(a11).pathname.replace(/\/+$/, "") || "/";
            return "/" !== b3;
          } catch (b3) {
            throw new bg(`Invalid base URL: ${a11}. Please provide a valid base URL.`);
          }
        }(a10)) return a10;
        let c2 = a10.replace(/\/+$/, "");
        return b2 && "/" !== b2 ? (b2 = b2.startsWith("/") ? b2 : `/${b2}`, `${c2}${b2}`) : c2;
      }
      ((a10) => {
        let b2 = a10?.disabled !== true, c2 = a10?.level ?? "error", d2 = a10?.disableColors !== void 0 ? !a10.disableColors : 1 !== function() {
          if (void 0 !== bk("FORCE_COLOR")) switch (bk("FORCE_COLOR")) {
            case "":
            case "1":
            case "true":
              return 4;
          ...
          if (void 0 !== bk("NODE_DISABLE_COLORS") && "" !== bk("NODE_DISABLE_COLORS") || void 0 !== bk("NO_COLOR") && "" !== bk("NO_COLOR") || "dumb" === bk("TERM")) return 1;
          if ("undefined" != typeof process && "win32" === process.platform || bk("TMUX")) return 24;
          if ("TF_BUILD" in bj && "AGENT_NAME" in bj) return 4;
          if ("CI" in bj) {
            for (let { 0: a11, 1: b3 } of bm) if (a11 in bj) return b3;
            return "codeship" === bk("CI_NAME") ? 8 : 1;
          }
          if ("TEAMCITY_VERSION" in bj) return null !== /^(9\.(0*[1-9]\d*)\.|\d{2,}\.)/.exec(bk("TEAMCITY_VERSION")) ? 4 : 1;
          switch (bk("TERM_PROGRAM")) {
            case "iTerm.app":
              if (!bk("TERM_PROGRAM_VERSION") || null !== /^[0-2]\./.exec(bk("TERM_PROGRAM_VERSION"))) return 8;
              return 24;
            case "HyperTerm":
            case "MacTerm":
              return 24;
          ...
          if ("truecolor" === bk("COLORTERM") || "24bit" === bk("COLORTERM")) return 24;
          if (bk("TERM")) {
            if (null !== /truecolor/.exec(bk("TERM"))) return 24;
            if (null !== /^xterm-256/.exec(bk("TERM"))) return 8;
            let a11 = bk("TERM").toLowerCase();
            if (bl[a11]) return bl[a11];
            if (bn.some((b3) => null !== b3.exec(a11))) return 4;
          }
          return bk("COLORTERM") ? 4 : 1;
        }();
        return { ...Object.fromEntries(bp.map((e2) => [e2, (...[f2, ...g2]) => ((e3, f3, g3 = []) => {
          if (!b2 || !(bp.indexOf(e3) <= bp.indexOf(c2))) return;
          let h2 = ((a11, b3, c3) => {
            let d3 = (/* @__PURE__ */ new Date()).toISOString();
            return c3 ? `${bo.dim}${d3}${bo.reset} ${bq[a11]}${a11.toUpperCase()}${bo.reset} ${bo.bright}[Better Auth]:${bo.reset} ${b3}` : `${d3} ${a11.toUpperCase()} [Better Auth]: ${b3}`;
          })(e3, f3, d2);
          if (!a10 || "function" != typeof a10.log) return void ("error" === e3 ? console.error(h2, ...g3) : "warn" === e3 ? console.warn(h2, ...g3) : console.log(h2, ...g3));
          a10.log("success" === e3 ? "info" : e3, f3, ...g3);
        })(e2, f2, g2)])), get level() {
          return c2;
        } };
      })(), new TextEncoder().encode, c(449), "undefined" == typeof URLPattern || URLPattern;
      var bs = c(814);
      ...
      let { env: bt, stdout: bu } = (null == (e = globalThis) ? void 0 : e.process) ?? {}, bv = bt && !bt.NO_COLOR && (bt.FORCE_COLOR || (null == bu ? void 0 : bu.isTTY) && !bt.CI && "dumb" !== bt.TERM), bw = (a10, b2, c2, d2) => {
        let e2 = a10.substring(0, d2) + c2, f2 = a10.substring(d2 + b2.length), g2 = f2.indexOf(b2);
        return ~g2 ? e2 + bw(f2, b2, c2, g2) : e2 + f2;
      }, bx = (a10, b2, c2 = a10) => bv ? (d2) => {
        let e2 = "" + d2, f2 = e2.indexOf(b2, a10.length);
        return ~f2 ? a10 + bw(e2, b2, c2, f2) + b2 : a10 + e2 + b2;
      ...
      async function bE(a10) {
        let b2 = ((a11, b3) => {
          b3?.cookiePrefix && (b3.cookieName ? b3.cookiePrefix = `${b3.cookiePrefix}-` : b3.cookiePrefix = `${b3.cookiePrefix}.`);
          let c3 = "headers" in a11 ? a11.headers : a11, d2 = a11 instanceof Request ? a11 : void 0;
          !function(a12, b4, c4, d3) {
            if (a12) return br(a12, b4);
            {
              let a13 = bj.BETTER_AUTH_URL || bj.NEXT_PUBLIC_BETTER_AUTH_URL || bj.PUBLIC_BETTER_AUTH_URL || bj.NUXT_PUBLIC_BETTER_AUTH_URL || bj.NUXT_PUBLIC_AUTH_URL || ("/" !== bj.BASE_URL ? bj.BASE_URL : void 0);
              if (a13) return br(a13, b4);
            }
            let e3 = c4?.headers.get("x-forwarded-host"), f3 = c4?.headers.get("x-forwarded-proto");
            if (e3 && f3) return br(`${f3}://${e3}`, b4);
            if (c4) {
              let a13 = function(a14) {
                try {
                  return new URL(a14).origin;
                } catch (a15) {
                  return null;
                }
              }(c4.url);
              if (!a13) throw new bg("Could not get origin from request. Please provide a valid base URL.");
              return br(a13, b4);
            }
            "undefined" != typeof window && window.location && br(window.location.origin, b4);
          }(d2?.url, b3?.path, d2);
          let e2 = c3.get("cookie");
          if (!e2) return null;
          let { cookieName: f2 = "session_token", cookiePrefix: g2 = "better-auth." } = b3 || {}, h2 = `${g2}${f2}`, i2 = `__Secure-${h2}`, j2 = function(a12) {
            let b4 = a12.split("; "), c4 = /* @__PURE__ */ new Map();
            return b4.forEach((a13) => {
              let [b5, d3] = a13.split("=");
              c4.set(b5, d3);
            }), c4;
          }(e2), k2 = j2.get(h2) || j2.get(i2);
          return k2 || null;
        })(a10), { pathname: c2 } = a10.nextUrl;
        return b2 || "/perfil" !== c2 ? S.next() : S.redirect(new URL("/auth", a10.url));
      }
      bx("\x1B[90m", "\x1B[39m"), bx("\x1B[40m", "\x1B[49m"), bx("\x1B[41m", "\x1B[49m"), bx("\x1B[42m", "\x1B[49m"), bx("\x1B[43m", "\x1B[49m"), bx("\x1B[44m", "\x1B[49m"), bx("\x1B[45m", "\x1B[49m"), bx("\x1B[46m", "\x1B[49m"), bx("\x1B[47m", "\x1B[49m"), bD(by("\u25CB")), bz(by("\u2A2F")), bB(by("\u26A0")), bD(by(" ")), bA(by("\u2713")), bC(by("\xBB")), new aT(1e4, (a10) => a10.length), /* @__PURE__ */ new WeakMap();
      let bF = { matcher: ["/perfil"] };
      Object.values({ NOT_FOUND: 404, FORBIDDEN: 403, UNAUTHORIZED: 401 });
      let bG = { ...f }, bH = bG.middleware || bG.default, bI = "/src/middleware";
      ...
      function bJ(a10) {
        return bf({ ...a10, page: bI, handler: async (...a11) => {
          try {
            return await bH(...a11);
          } catch (e2) {
...
var init_edgeFunctionHandler = __esm({
  "node_modules/@opennextjs/aws/dist/core/edgeFunctionHandler.js"() {
    ...
    globalThis._ROUTES = [{ "name": "src/middleware", "page": "/", "regex": ["^(?:\\/(_next\\/data\\/[^/]{1,}))?\\/perfil(\\.json)?[\\/#\\?]?$"] }];
